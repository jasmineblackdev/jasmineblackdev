# 👋 Hi, I'm Jasmine Black  

**Front-End & Mobile Developer in Progress | Software Engineering Student @ WGU | UX/UI Designer**

I’m transitioning from senior design and UX roles into software engineering with a focus on **front-end development, mobile applications, and user-centered digital experiences**. I’m currently earning my **B.S. in Software Engineering (Java Track)** at Western Governors University while building real projects to expand my technical foundation.

I blend **strong visual design**, **clean interface development**, and **modern user experience principles** to create products that look good and feel good to use.

---

## 🔥 What I'm Focused On
- Front-End Development (HTML, CSS, JavaScript)  
- Mobile Application Development  
- UI/UX Design & Product Design  
- Python & Java fundamentals (WGU coursework)  
- Building clean, responsive interfaces  
- Transforming design systems into working interactive products  

---

## 🛠️ Tech & Tools
**Development:** HTML • CSS • Bootstrap • Git • GitHub • VS Code  
**Learning:** Python • Java • Mobile Development  
**Design:** Figma • Adobe XD • Illustrator • Photoshop • InDesign  
**Specialties:** Wireframing • UI Layout • UX Research • Style Guides • Branding  

---

## 🚀 Featured Projects

### 🔹 **Personal Portfolio**  
A modern developer portfolio showcasing my transition from UI/UX to software engineering.  
**Live** https://jasmineblack.vercel.app
**Repo** https://github.com/jasmineblackdev/jasmineblack_portfolio.git

---

### 🔹 **Tindog – Responsive Landing Page**  
A playful, fully responsive landing page built with HTML, CSS, and Bootstrap.  
**Live:** https://jasmineblackdev.github.io/Tindog/  
**Repo:** https://github.com/jasmineblackdev/Tindog  

---

### 🔹 **Sentinel Home Health – Concept Site**  
A UI/UX-driven home health concept site demonstrating practical design-to-code workflow.  
**Repo:** https://github.com/jasmineblackdev/SentinelHomeHealth  

---

### 🔹 **Python Fitness Toolkit (In Progress)**  
A set of small Python tools for macros, calorie targets, hydration, and workout logging as I learn core programming through WGU.

---

## 🎓 WGU – Software Engineering (Java Track)

Currently building a strong foundation in:
- Python & Java  
- Data structures & algorithms  
- Software design principles  
- Full-stack concepts  
- Mobile development fundamentals  

My goal is to create portfolio-ready projects that reflect both my **design background** and my growing **engineering skillset**.

---

## 📫 Let’s Connect  
**Email:** [jazzblackartist@gmail.com](mailto:jazzblackartist@gmail.com)  
**GitHub:** https://github.com/jasmineblackdev  
**UX Portfolio:** https://jasmineblack.vercel.app
*(LinkedIn coming soon)*

---

## ✨ Fun Facts  
- I love fitness and plan to build fitness-focused tech tools  
- I enjoy taking complex designs and turning them into clean code  
- I’m passionate about mobile-first experiences  
- My background gives me the rare mix of **designer + developer**  

---

Thanks for visiting my profile! 💙  
Feel free to explore my repos or reach out if you'd like to collaborate.
